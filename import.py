...
if __name__=="__main__":
            ...
            for day in tqdm(days_of_month):
                for hour in hours:
                    for minute in minutes:
                        try:
                            url = "http://data.gdeltproject.org/gdeltv2/"+year+month+day+hour+minute+".translation.mentions.CSV.zip"
                            r = requests.get(url, allow_redirects=True)
                            open('gdelt_data/'+year+month+day+hour+minute+".translation.mentions.CSV.zip", 'wb').write(r.content)
                            with zipfile.ZipFile('gdelt_data/'+year+month+day+hour+minute+".translation.mentions.CSV.zip", 'r') as zip_ref:
                                    zip_ref.extractall("gdelt_data/")
                            os.remove('gdelt_data/'+year+month+day+hour+minute+".translation.mentions.CSV.zip")
                        except:
        ...
        for i in range(len(months)):
            ...
            for day in tqdm(days_of_month):
                for hour in hours:
                    for minute in minutes:
                        try:
                            gdelt_events = pd.read_csv("gdelt_data/"+year+month+day+hour+minute+".translation.mentions.CSV", delimiter='\t', names=col_names_mentions)
                            urls = gdelt_events["MentionIdentifier"].tolist()
                            srclc = gdelt_events["SRCLC"].tolist()
                            fr_urls = []
                            for i, lang in enumerate(srclc):
                                if "srclc:fra" in lang:
                                    fr_urls.append(urls[i])
                            urls_clean = [tldextract.extract(url).registered_domain for url in fr_urls]
                            urls_to_process = []
                            is_fake = []
                            for i in range(len(urls_clean)):
                                ...
                                if url in unreliable_clean:
                                    ...
                                    is_fake.append(0)
                                urls_to_process.append(fr_urls[i])
    ...
    pickle.dump(df, open("urls_to_download_fr.pkl", "wb"))
